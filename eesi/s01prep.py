...
class Preprocess:
    ...
    def preprocess(cls, src: str | Path):
        ...
        if Vars.REGISTRATION_DATE in schema:
            lf = lf.with_columns(pl.col(Vars.REGISTRATION_DATE).dt.date())

        if Vars.SUPPORT_TYPE in schema:
            lf = lf.with_columns(
                # NOTE 보호유형: 데이터 정제 어려움
                pl.col(Vars.SUPPORT_TYPE).str.replace_all(' ', '').str.split(',')
            )

        if Vars.EXISTING_BOILER in schema:
            lf = lf.with_row_index()
            fuel = (
                lf.select(['index', *Vars.EXISTING_BOILER_FUEL_COLS])
                .unpivot(index='index', variable_name=Vars.EXISTING_BOILER_FUEL)
                .filter(pl.col('value').is_not_null())
                .drop('value')
            )
            lf = (
                lf.drop(Vars.EXISTING_BOILER_FUEL_COLS)
                .select(pl.all().exclude(Vars.EXISTING_BOILER), Vars.EXISTING_BOILER)
                .with_columns(
                    pl.col(Vars.EXISTING_BOILER).replace_strict(
